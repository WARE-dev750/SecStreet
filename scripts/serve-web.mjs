...
if (!projectRoot) { console.error("usage: node scripts/serve-web.mjs <project-root> [port] [host]"); process.exit(2); }
const port = process.argv[3] ? Number(process.argv[3]) : 5050;
const host = process.argv[4] || "0.0.0.0";
const h = await startWebServer({ projectRoot, port, host });
console.log("web IDE at http://127.0.0.1:" + h.port);
